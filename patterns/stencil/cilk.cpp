...
void apply_stencil(const int radius, const double stddev, const int rows, const int cols, pixel * const in, pixel * const out) {
    ...
    double* intensities = (double*)malloc(sizeof(double)*rows*cols);
    ...
    cilk_for(int i = 0; i < rows; ++i) {
	    ...
	    intensities[out_offset] = (out[out_offset].red+out[out_offset].green+out[out_offset].blue)/3.0;
	...
	for (int j = 0; j<cols; j++){
		...
		for (int y = j-1, ky = 0; y<=j+1; y++,ky++){
			...
			colorx+=kernelx[k_offset]*intensities[in_offset];
			colory+=kernely[k_offset]*intensities[in_offset];
		    }
		}
	    }
	    double color = sqrt(colorx*colorx+colory*colory);
	    out[out_offset].red += color;
	    out[out_offset].green += color;
	    out[out_offset].blue += color;
	}
    }
    free(intensities);
}
